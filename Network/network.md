...
Cross-site scripting 跨域脚本。服务器将攻击者存储的数据原原本本的显示给别的用户。比如论坛用户发布的内容带有 script 脚本。预防：前端过滤，后端转义。

# IO 多路复用

单线程或单进程同时监测若干个文件描述符是否可以执行 IO 操作的能力。与多线程/进程相比，可以使用更少的资源完成更多的事情。
...
一个线程不断接受客户端连接，并把 socket 文件描述符放到一个 list 里，另一个线程不再自己遍历，而是调用 select，将这批文件描述符 list 交给操作系统去遍历。

做到了一个线程处理多个客户端连接（文件描述符），又减少了系统调用的开销（多个文件描述符只有一次 select 的系统调用 + n 次就绪状态的文件描述符的 read 系统调用）。

![select](../assets/Network/Select.jpg)

## poll

它和 select 的主要区别就是，去掉了 select 只能监听 1024 个文件描述符的限制。底层使用链表而非数组
